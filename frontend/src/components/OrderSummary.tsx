...
import { OrderItemState, AppliedChargeState } from '../types/domain';
...
export const OrderSummary: React.FC = () => {
  ...
  const handleDecrementQuantity = (variantId: string) => {
    const item = state.items.get(variantId);
    if (item) {
      // The reducer will handle removing the item if quantity becomes 0 or less
      dispatch({ type: 'UPDATE_QUANTITY', payload: { variantId, quantity: item.quantity - 1 } });
    }
  };
  ...
  const qtyButtonClasses = "px-2 py-0.5 border border-gray-300 bg-gray-100 hover:bg-gray-200 text-brand-text-on-light rounded transition-colors text-sm font-medium";
  ...
  return (
    <div className="p-4 md:p-6 bg-white rounded-lg shadow-lg">
      ...
      {orderItemsArray.length > 0 && (
        <div className="mb-4">
          ...
          <ul className="space-y-2">
            {orderItemsArray.map((item: OrderItemState) => (
              <li key={item.variantId} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                ...
                <div className="flex items-center space-x-2 shrink-0">
                  ...
                  <span className="w-8 text-center text-brand-text-on-light font-medium tabular-nums"> {/* Adjusted width for quantity */}
                    {item.quantity}
                  </span>
                  <button 
      ...
      {/* ... (Applied Charges and Totals sections remain the same as before) ... */}
      {appliedChargesArray.length > 0 && (
        <div className="mb-4 pt-2 border-t border-gray-100">
          ...
          {appliedChargesArray.map((charge: AppliedChargeState) => (
