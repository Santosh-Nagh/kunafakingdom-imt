...
import React, { useState } from 'react'; // React, useState are value imports
import { useOrder } from '../contexts/OrderContext';

// DetailField is a local type alias, not an import that needs 'import type'
...
export const OrderDetailsInput: React.FC = () => {
  ...
  const handleInputChange = (field: DetailField, value: string) => {
    ...
    dispatch({ type: 'UPDATE_CUSTOMER_DETAIL', payload: { field, value } });
        ...
        <div>
          ...
          />
           {state.aggregatorId.length >= MAX_AGGREGATOR_ID_LENGTH && (
            <p className="mt-1 text-xs text-orange-500">Max length reached for Aggregator ID.</p>
          )}
        </div>
        <div>
          ...
          {state.notes.length >= MAX_NOTES_LENGTH && (
            <p className="mt-1 text-xs text-orange-500">Max length reached for Notes.</p>
          )}
        </div>
      </div>
    </div>
